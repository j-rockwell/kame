...
export const NewCustomerInput = ({
  ...
  const handleValidate = useCallback(() => {
    ...
    if (email.value.length < 3 || email.value.length > 64) {
      setEmail({
        value: email.value,
        error: 'Email address must be 3-64 characters',
      });
      valid = false;
    }
    ...
    if (phoneMiddle.value.length !== 3) {
      setPhoneMiddle({
        value: phoneMiddle.value,
        error: 'Invalid middle part',
      });
      valid = false;
    }
  ...
  const handleDataUpdate = useCallback(
    (field: InputField, value: string, clearErrors?: boolean) => {
      ...
      if (
        field.startsWith('phone_') &&
        (Number.isNaN(value) || Number.isNaN(parseInt(value)))
      ) {
        return;
      }
      ...
      switch (field) {
        case 'first_name':
          setFirstName({
            value: value,
            error: clearErrors ? undefined : firstName.error,
          });
          break;
        case 'last_name':
          setLastName({
            value: value,
            error: clearErrors ? undefined : lastName.error,
          });
          break;
        case 'email_address':
          setEmail({
        ...
        case 'phone_left':
          setPhoneLeft({
            value: value,
            error: clearErrors ? undefined : phoneLeft.error,
          });
          break;
        case 'phone_middle':
          setPhoneMiddle({
            value: value,
            error: clearErrors ? undefined : phoneMiddle.error,
          });
          break;
        case 'phone_right':
          setPhoneRight({
            value: value,
            error: clearErrors ? undefined : phoneRight.error,
          });
          break;
        case 'password':
          setPassword({
        ...
        case 'confirm_password':
          setConfirmPassword({
            value: value,
            error: clearErrors ? undefined : confirmPassword.error,
          });
          break;
      }
    },
  ...
  return (
    <motion.div
      ...
      <VStack spacing={'2rem'} mt={'2rem'}>
        <FormControl
          isInvalid={
            firstName.error !== undefined && firstName.error?.length > 0
          }>
        ...
        <FormControl
          isInvalid={
            lastName.error !== undefined && lastName.error?.length > 0
          }>
        ...
        <FormControl
          isInvalid={email.error !== undefined && email.error?.length > 0}>
          ...
          <HStack>
            <FormControl w={'25%'} isInvalid={phoneLeft.error !== undefined}>
              <Input
                ...
                maxLength={3}
                onChange={e =>
                  handleDataUpdate('phone_left', e.target.value, true)
                }
                {...inputStyling}
              />
            </FormControl>
            ...
            <FormControl w={'25%'} isInvalid={phoneMiddle.error !== undefined}>
              <Input
                ...
                maxLength={3}
                onChange={e =>
                  handleDataUpdate('phone_middle', e.target.value, true)
                }
                {...inputStyling}
              />
            </FormControl>
            ...
            <FormControl w={'50%'} isInvalid={phoneRight.error !== undefined}>
              <Input
                ...
                maxLength={4}
                onChange={e =>
                  handleDataUpdate('phone_right', e.target.value, true)
                }
                {...inputStyling}
              />
            </FormControl>
        ...
        <FormControl
          isInvalid={
            password.error !== undefined && password.error?.length > 0
          }>
        ...
        <FormControl
          ...
          <Input
            type={showPassword ? 'text' : 'password'}
            onChange={e =>
              handleDataUpdate('confirm_password', e.target.value, true)
            }
            {...inputStyling}
          />

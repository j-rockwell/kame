...
import {
  GetAvailableMenusRequest,
  GetAvailableMenusResponse,
} from '@/requests/types/Menu';
...
export async function getMenuAvailability(
  ...
  return new Promise<GetAvailableMenusResponse>(async (resolve, reject) => {
    try {
      const res = await axios.get<GetAvailableMenusResponse>(
        `${API_URL}/menu/availability${params}`,
      );
